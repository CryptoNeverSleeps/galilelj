...
import org.galilelj.core.listeners.BlockChainListener;
import org.galilelj.store.FlatDB;
import org.galilelj.store.HashStore;
import org.galilelj.store.MasternodeDB;
 ...
 * galilelj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating galilelj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use galilelj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a galilelj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using galilelj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing galilelj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.galilelj.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.galilelj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.galilelj.core.NetworkParameters} specified when this context was (auto) created. The
    ...
    public void initGalilel(boolean liteMode, boolean allowInstantX) {
    ...
    public void setPeerGroupAndBlockChain(PeerGroup peerGroup, @Nullable AbstractBlockChain chain)
    {
        ...
        if (sporkManager!=null) {
            ...
            log.error("##### Galilel init not called!, this is going to be an issue in the future");
        }
        if (instantSend!=null) {
    ...
    BlockChainListener updateHeadListener = new BlockChainListener () {
        ...
        {
            //todo mbroemme: commented galilel MN stuff
