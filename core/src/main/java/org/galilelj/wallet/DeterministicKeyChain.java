...
import org.galilelj.core.BloomFilter;
...
import org.galilelj.utils.Threading;
 ...
 * {@link org.galilelj.crypto.DeterministicHierarchy}, to derive all the keys in the keychain from a master seed.
 ...
 * written down on paper or, when represented using a BIP 39 {@link org.galilelj.crypto.MnemonicCode},
 ...
 * {@link org.galilelj.crypto.DeterministicKey#serializePubB58(org.galilelj.core.NetworkParameters)}. The resulting "xpub..." string encodes
 * sufficient information about the account key to create a watching chain via
 * {@link org.galilelj.crypto.DeterministicKey#deserializeB58(org.galilelj.crypto.DeterministicKey, String, org.galilelj.core.NetworkParameters)}
 * (with null as the first parameter) and then
 * {@link DeterministicKeyChain#DeterministicKeyChain(org.galilelj.crypto.DeterministicKey,KeyChainType keyChaintype)}.</p>
 *
 * <p>This class builds on {@link org.galilelj.crypto.DeterministicHierarchy} and
 * {@link org.galilelj.crypto.DeterministicKey} by adding support for serialization to and from protobufs,
...
public class DeterministicKeyChain implements EncryptableKeyChain {
    ...
    public static final ChildNumber GALI_PATH = new ChildNumber(119,true);
    public static final ImmutableList<ChildNumber> BIP44_ACCOUNT_ZERO_PATH =
            ImmutableList.of(BIP44_MASTER_KEY, GALI_PATH, ChildNumber.ZERO_HARDENED);
    ...
    public static class Builder<T extends Builder<T>> {
        ...
        public DeterministicKeyChain build() {
            ...
            if (random != null) {
                ...
                chain = new DeterministicKeyChain(watchingKey,KeyChainType.BIP44_GALI_ONLY);
            }
    ...
    protected DeterministicKeyChain(DeterministicSeed seed) {
        this(seed, null,KeyChainType.BIP44_GALI_ONLY);
    }
    ...
    public DeterministicKeyChain(DeterministicKey watchingKey,KeyChainType keyChainType) {
        ...
        if (keyChainType != KeyChainType.BIP44_GALI_ONLY)
    ...
    protected DeterministicKeyChain(DeterministicKey watchKey, boolean isFollowing) {
        this(watchKey,KeyChainType.BIP44_GALI_ONLY);
    ...
    public static DeterministicKeyChain watch(DeterministicKey accountKey) {
        return new DeterministicKeyChain(accountKey,KeyChainType.BIP44_GALI_ONLY);
    }
    ...
    private void initializeHierarchyUnencrypted(DeterministicKey baseKey) {
        if (baseKey.isPubKeyOnly() && keyChainType == KeyChainType.BIP44_GALI_ONLY){
    ...
    public List<DeterministicKey> getKeys(KeyPurpose purpose, int numberOfKeys) {
        ...
        try {
            ...
            for (int i = 0; i < numberOfKeys; i++) {
                ...
                // by any real problem reports from galilelj users, but I've heard of cases via the grapevine of
     ...
     * The returned key can be serialized and then passed into {@link #watch(org.galilelj.crypto.DeterministicKey)}
    ...
    public static List<DeterministicKeyChain> fromProtobuf(List<Protos.Key> keys, @Nullable KeyCrypter crypter, KeyChainFactory factory) throws UnreadableWalletException {
        ...
        KeyChainType keyChainType = KeyChainType.BIP44_GALI_ONLY;
        ...
        for (Protos.Key key : keys) {
            ...
            if (!path.isEmpty() && path.get(0).equals(BIP44_MASTER_KEY)){
                // is BIP44
                keyChainType = KeyChainType.BIP44_GALI_ONLY;
                break;
            }
        }
    ...
    public DeterministicKeyChain toEncrypted(KeyCrypter keyCrypter, KeyParameter aesKey) {
        return toEncrypted(keyCrypter,aesKey,KeyChainType.BIP44_GALI_ONLY);
    }
     ...
     * @see org.galilelj.wallet.MarriedKeyChain
     */
